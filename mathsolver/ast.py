...
class Lit(Expr):
  ...
  def __str__(self, outside=True):
    if self.fraction < 0 and outside == False:
...
class Op(Expr):
  ...
  def thoroughApply(self, f):
    if all([e.thoroughApply(f) is None for e in self.args]):
      ...
      return Op(self.name, [e if e.thoroughApply(f) is None else e.thoroughApply(f) for e in self.args])
  ...
  def __eq__(self, other):
    return isinstance(other, Op) and self.name == other.name and self.args == other.args
  ...
  def __str__(self, outside=True):
    def prettyFunction():
      return self.name + '(' + ', '.join([e.__str__(outside=False) for e in self.args]) + ')'
    def prettyOperator():
      if self.name == '*':
        t = ''
