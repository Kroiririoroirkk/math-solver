...
def litsGreatestCmp(e1, e2):
    if isinstance(e1, Lit) and isinstance(e2, Lit):
        ...
        comparedLists = [litsGreatestCmp(e1.args[i], e2.args[i])
                         for i in range(min(len(e1.args), len(e2.args)))
                         if litsGreatestCmp(e1.args[i], e2.args[i]) != 0]
        if comparedLists:
...
def groupingPropAddition(e):
    if isinstance(e, Op) and e.name == '+':
        expandedSumList = reduce(list.__add__,
                                 [subExpr.args
...
def groupingPropMultiplication(e):
    if isinstance(e, Op) and e.name == '*':
        expandedProductList = reduce(list.__add__,
                                     [subExpr.args
                                      if isinstance(subExpr, Op)
                                         and subExpr.name == '*'
                                      else [subExpr]
    ...
    if isinstance(e, Op) and e.name == '*':
        try:
            sum = [factor
                   ...
                   if isinstance(factor, Op) and factor.name == '+'][0]
            argsWithoutSum = e.args[:]
            argsWithoutSum.remove(sum)
            return Op('+', [Op('*', [summand] + argsWithoutSum)
...
def distributivePropExponentiationOverMultiplication(e):
    if isinstance(e, Op) \
       and e.name == '^' \
       and len(e.args) == 2 \
       and isinstance(e.args[0], Op) \
       and e.args[0].name == '*':
        return Op('*', [Op('^', [factor, e.args[1]])
                        for factor in e.args[0].args])
...
def powerOfAPowerProp(e):
    if isinstance(e, Op) \
       ...
       and len(e.args[0].args) == 2:
        return Op('^', [e.args[0].args[0],
                        Op('*', [e.args[0].args[1], e.args[1]])])
    elif (isinstance(e, Op)
          and e.name == 'root'
          and len(e.args) == 2
          and isinstance(e.args[0], Op)
          and e.args[0].name == 'root'
          and len(e.args[0].args) == 2):
        return Op('root', [e.args[0].args[0],
                           Op('*', [e.args[0].args[1], e.args[1]])])
