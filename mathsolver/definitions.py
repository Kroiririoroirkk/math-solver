...
def reduceThorough(f, es):
    combinations = [(f(es[0], e), e)
                    for e in es[1:]
                    if f(es[0], e) is not None]
    if combinations:
...
def addExprs(e1, e2):
    if isinstance(e1, Lit) and isinstance(e2, Lit):
    ...
    elif (isinstance(e1, Op)
          and e1.name == '*'
          and len(e1.args) > 1
          and isinstance(e2, Op)
          and e2.name == '*'
          and len(e2.args) > 1):
        if e1.args[0] == e2.args[0]:
            added = addExprs(e1.args[1] if len(e1.args) == 2
                             else Op('*', e1.args[1:]),
                             e2.args[1] if len(e2.args) == 2
                             else Op('*', e2.args[1:]))
            if added:
        ...
        if e1 == e2.args[0]:
            added = addExprs(e2.args[1] if len(e2.args) == 2
                             else Op('*', e2.args[1:]),
                             Lit(1))
            if added:
    ...
    if isinstance(e, Op) \
       ...
       and isinstance(e.args[1], Lit):
        if e.args[1].fraction.denominator == 1 and isinstance(e.args[0], Lit):
            ...
            return Op('root',
                      [Op('^', [e.args[0], e.args[1].fraction.numerator]),
                       Lit(e.args[1].fraction.denominator)])
...
def root(e):
    if isinstance(e, Op) \
       and e.name == 'root' \
       and isinstance(e.args[1], Lit) \
       and e.args[1].fraction.denominator == 1:
        if isinstance(e.args[0], Lit) and e.args[0].fraction.denominator == 1:
            ...
            if val != 1:
                return Op('*', [Lit(val),
                                Op('root', [Lit(radicand), Lit(index)])])
        elif (isinstance(e.args[0], Op)
              and e.args[0].name == '^'
              and isinstance(e.args[0].args[1], Lit)):
            var = e.args[0].args[0]
            ...
            factorList = [root(Op('root', [expr, e.args[1]]))
                          if root(Op('root', [expr, e.args[1]]))
                          else Op('root', [expr, e.args[1]])
                          for expr in e.args[0].args]
            if [Op('root', [factor, e.args[1]]) for factor in e.args[0].args] \
               != factorList:
