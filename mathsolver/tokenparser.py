    ...
    def listToASTRecursive(tokens, stack, arityStack, arity, acc):
        ...
        def popOperatorsFromStack(c):
            if stack:
                ...
                stack.append(op)
                if op.tokenType == TokenType.OPERATOR:
                    isLeftAssoc = assoc[c] == Associativity.LEFT
                    overPrecedence = precedence[op.content] > precedence[c]
                    if isLeftAssoc and precedence[op.content] >= precedence[c]:
                        popTopFromStack()
                        popOperatorsFromStack(c)
                    elif not isLeftAssoc and overPrecedence:
                        popTopFromStack()
                        popOperatorsFromStack(c)
                elif op.tokenType == TokenType.FUNCTION:
                    popTopFromStack()
        ...
        if tokens:
            fromToken(tokens[0])
            return listToASTRecursive(tokens[1:],
                                      stack,
                                      arityStack,
                                      arity,
                                      acc)
